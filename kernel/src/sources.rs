...
use mork_expr::{Expr, ExprZipper, Tag, byte_item, destruct, item_byte, serialize};
use pathmap::PathMap;
...
impl Source for HeadSource {
    fn new(e: Expr) -> Self {
        ...
        let max: usize = str::from_utf8(max_s)
            ...
            .expect("a number");
        assert_ne!(max, 0);
        let skip = 1 + 1 + 4 + 1 + max_s.len();
        let span = unsafe { e.span().as_ref().unwrap() };
        let target_prefix = unsafe {
            e.prefix()
                .unwrap_or_else(|_| slice_from_raw_parts(e.ptr, span.len() - 1))
                .as_ref()
                .unwrap()
        }[skip..]
            .to_vec();
        HeadSource {
            ...
            skip,
            max,
            prefix: span[..skip].to_vec(),
            target_prefix,
        }
    }

    fn request(&self) -> impl Iterator<Item = ResourceRequest> {
        trace!(target: "source", "head requesting {}", serialize(&self.target_prefix[..]));
        std::iter::once(ResourceRequest::BTM(Box::leak(
            self.target_prefix.clone().into_boxed_slice(),
        )))
    }
    ...
    where
        'path: 'trie,
    {
        let Resource::BTM(mut rz) = it.next().unwrap() else {
            unreachable!()
        };
        let mut selected = PathMap::new();
        let mut count = 0usize;
        let mut top = Vec::new();

        while rz.to_next_val() {
            let path = rz.origin_path();
            let rel = if path.starts_with(&self.target_prefix[..]) {
                &path[self.target_prefix.len()..]
            } else {
                path
            };
            if count == self.max {
                ...
                selected.remove(&top[..]);
                selected.insert(rel, ());
                let mut srz = selected.read_zipper();
                srz.descend_last_path();
                top.clear();
                top.extend_from_slice(srz.path());
            } else if selected.insert(rel, ()).is_none() {
                if &top[..] <= rel {
                    top.clear();
                    top.extend_from_slice(rel);
                }
                count += 1;
            }
        }
        ...
        let mut srz = selected.read_zipper();
        while srz.to_next_val() {
            let mut full = Vec::new();
            full.extend_from_slice(&self.prefix[..]);
            full.extend_from_slice(&self.target_prefix[..]);
            full.extend_from_slice(srz.origin_path());
            out.insert(&full[..], ());
        }
        AFactor::HeadSource(out.into_read_zipper(&[]))
    }
}
    ...
    fn new(e: Expr) -> Self {
        if unsafe {
            ...
            *e.ptr == item_byte(Tag::Arity(3))
            ...
            ASource::HeadSource(HeadSource::new(e))
        } else {
            unreachable!()
        }
    }
